...
**Важно**: Данная архитектура адаптирована под идиоматический стиль NestJS, где некоторые слои Clean Architecture интегрированы с возможностями фреймворка для достижения баланса между чистотой архитектуры и практической эффективностью.
...
### Адаптация Clean Architecture для NestJS

Проект следует принципам Чистой Архитектуры с учетом особенностей NestJS:

1. **Интеграция с фреймворком**: Бизнес-логика использует DI контейнер NestJS, но не зависит от HTTP слоя
2. **Тестируемость**: Use Cases и сервисы можно тестировать изолированно через моки зависимостей
3. **Разделение ответственности**: Контроллеры обрабатывают HTTP, сервисы координируют Use Cases
4. **Абстракция БД**: Доменный слой работает через интерфейс репозиториев, не зная о Mongoose
5. **Слабая связанность**: Модули взаимодействуют через четко определенные интерфейсы
